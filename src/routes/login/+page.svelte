<script lang="ts">
	...
	async function handleLogin(e: Event) {
		...
		const { data: signInData, error: authError } = await data.supabase.auth.signInWithPassword({
			email,
			password
		});
		...
		const { data: factorsData } = await data.supabase.auth.mfa.listFactors();
		
		if (factorsData?.totp && factorsData.totp.length > 0) {
			// User has MFA enabled - find verified factor
	...
	async function handleMfaVerify(e: Event) {
		...
		try {
			...
			const { data: challengeData, error: challengeError } = await data.supabase.auth.mfa.challenge({
...
<div class="min-h-screen flex items-center justify-center p-6">
	<div class="w-full max-w-md">
		<div class="text-center mb-8">
			...
			{#if mfaRequired}
				<h1 class="font-display text-2xl font-bold mb-2">Two-Factor Authentication</h1>
				<p class="text-sw-text-dim">Enter the code from your authenticator app</p>
			{:else}
			...
			{#if mfaRequired}
				...
				<form onsubmit={handleMfaVerify} class="space-y-5">
					...
					<div class="text-center">
						<div class="w-16 h-16 rounded-2xl bg-sw-accent/10 flex items-center justify-center mx-auto mb-4">
							<i class="fa-solid fa-shield-halved text-2xl text-sw-accent"></i>
						</div>
						<p class="text-sm text-sw-text-dim mb-6">
							Open your authenticator app and enter the 6-digit code for SpentWorth.
						</p>
					</div>
					...
					<div class="flex justify-center">
						<input
							...
							class="input text-center text-2xl font-mono tracking-[0.5em] w-48"
							placeholder="000000"
						/>
					</div>

					<button type="submit" disabled={mfaVerifying || mfaCode.length !== 6} class="btn btn-primary w-full py-3">
						{#if mfaVerifying}
				...
				<div class="mt-6 text-center">
					<button
						type="button"
						onclick={backToLogin}
						class="text-sm text-sw-text-dim hover:text-sw-accent transition-colors"
					>
